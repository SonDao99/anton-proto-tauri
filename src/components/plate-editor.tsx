...
import {
  ...
  CitationElement,
  ELEMENT_CITATION,
} from "@/components/citation-plugin";
import { CitationModal } from "@/components/CitationModal";
import { Citation } from "@/types/ward-round";
import { deserializeMd } from "@platejs/markdown";

// Hardcoded thread ID for external agent communication
const ACTIVE_THREAD_ID = "active-session";
  ...
  const handleMessage = useCallback(
    (message: any) => {
      if (message.type === "note_complete") {
        ...
        // Update editor using proper Slate transforms API
        // This ensures React and Slate stay in sync, which is essential for drag & drop
        editor.tf.replaceNodes(withCitations, {
          at: [],
          children: true,
        });
        ...
        console.log(
          "Editor updated with",
          Object.keys(newCitations).length,
          "citations",
        );
      }
    },
  ...
  const handleEditorClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (citationElement) {
        ...
        const citationNum = citationElement.getAttribute(
    ...
    <>
      <Plate editor={editor}>
        {/*<div className="px-3 py-2 text-xs text-muted-foreground flex gap-4">
          ...
          />
          <div>
            WS: {connected ? "✓ connected" : "✗ disconnected"} • Stream:{" "}
        ...
        <div onClick={handleEditorClick}>
          <EditorContainer>
            <Toolbar className="flex justify-start gap-1 rounded-t-lg border-primary/30">
              <ToolbarGroup>
      ...
      for (const part of parts) {
        ...
        if (match) {
          ...
          if ("strikethrough" in child)
            textNode.strikethrough = child.strikethrough;
