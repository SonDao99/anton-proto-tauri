...
export class MarkdownJoiner {
  ...
  private isTrivialText(s: string): boolean {
    const t = s;
    if (t === '\n') return true;
  ...
  private isTrivialBuffer(): boolean {
    return this.isTrivialText(this.buffer);
  }
  ...
  flush(): string {
    ...
    this.clearBuffer();
    return this.isTrivialText(remaining) ? '' : remaining;
  }
  ...
  processText(text: string): string {
    ...
    for (const char of text) {
      if (
        ...
        if (char === '\n') {
          // Flush buffer content (if non-trivial)
          if (!this.isTrivialBuffer()) {
            output += this.buffer;
          }
          // CRITICAL: Always output the newline character itself!
          output += '\n';
          this.clearBuffer();
        }
        ...
        if (
          ...
          this.clearBuffer();
          // If the triggering char was a newline, output it
          if (char === '\n') {
            output += '\n';
          } else {
            // Otherwise output the triggering character
            output += char;
          }
        }
      } else {
